...
snp2gene <- unique(snp2gene[REPORTED_OR_MAPPED != "r"])
...
```
## Gene name discrepancies

```{r}
sprintf("Gene name discrepancies: %d (%.1f%%)", nrow(snp2gene[geneName!=EnsemblName]), 100*nrow(snp2gene[geneName!=EnsemblName])/nrow(snp2gene))
knitr::kable(snp2gene[geneName!=EnsemblName][sample(.N, 12)], caption="Gene name discrepancies (examples)")
```

Also check TCRD xrefs.

```{r}
tcrd <- read_delim("data/tcrd_targets.tsv", "\t", col_types=cols(.default=col_character(), idgList=col_logical()), na = c("", "NA", "NULL"))
setDT(tcrd)
tcrd <- tcrd[, .(tcrdProteinId, TDL, ncbiGeneId, uniprotId, ensemblGeneId)]
snp2gene <- merge(snp2gene, tcrd, by.x="ensemblId", by.y="ensemblGeneId", all.x=T)
sprintf("ensemblGeneIds NOT mapped to TCRD: %d (%.1f%%)", 
        sum(is.na(snp2gene$tcrdProteinId)),
        100*sum(is.na(snp2gene$tcrdProteinId))/nrow(snp2gene))
```
