...
### tiga_gt_variables.R - Produce gt_variables.tsv.gz, input for
### tiga_gt_stats.R, to produce gt_stats.tsv.gz, for TIGA Shiny app.
### ~18hr
#############################################################################
### SEE FOR ALL INPUT WORKFLOWS: Go_gwascat_GetData.sh
#############################################################################
# Now using avg-rank or median-rank instead of mu_scores.
...
if (length(args)==5) {
  ...
  message("ERROR: Syntax: tiga_gt_variables.R GWASFILE COUNTSFILE ASSNFILE SNP2GENEFILE TRAITFILE ICITEFILE TCRDFILE ENSEMBLFILE OFILE OFILE_PROVENANCE\n...or... no args for defaults")
  quit()
}
