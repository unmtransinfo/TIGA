...
anns <- c(sprintf("N_pmid = %d", nrow(gwas_icite)), sprintf("mean = %.1f ; median = %.1f ; max = %.1f", mean(gwas_icite$relative_citation_ratio, na.rm=T), median(gwas_icite$relative_citation_ratio, na.rm=T), max(gwas_icite$relative_citation_ratio, na.rm=T)), sprintf("90%%ile = %.1f", qtl["90%"]), (sprintf("(Plot truncated at %d.)", xmax)))
plot_ly(gwas_icite, x=~relative_citation_ratio, type="histogram", marker=list(color="#22AA22")) %>%
  layout(title = 'Relative Citation Ratio (RCR) distribution',
         xaxis = list(title="RCR", type="normal", range=c(0, xmax), showgrid=T),
