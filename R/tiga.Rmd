...
```{r}
ifile_gwas <- "data/gwascat_gwas.tsv.gz"
...
ifile_efo2do <- "data/oxo_efo2doid_d2.tsv"
ifile_efo_gwas <- "data/efo_sub_gwas.tsv"
gt_provfile <- "data/gt_provenance.tsv.gz"
...
We estimate RCR=median as reasonable prior for new publications and any lacking RCR from iCite. 
...
plot_ly(gwas_icite, 
  ...
  layout(title = 'iCite Relative Citation Ratio<br>for top journals by GWAS publication count',
         xaxis = list(title="", range=c(2016, 2020), showgrid=F, dtick=1, tickangle=45, tickfont=list(size=14)),
...
The API provides additional data and/or rigorous formatting (JSON) for ___mapped genes___. Using Ensembl IDs, we query 
the Ensembl API for additional annotations including gene ___biotype___, thereby filtering for __protein_coding__.
...
efo_sub <- merge(efo_sub, trait_counts[, .(efoId, trait_N_study=N_study, trait_name=MAPPED_TRAIT)], by="efoId", all.x=T, all.y=F)
efo_sub <- merge(efo_sub, trait_counts[, .(efoId, subclass_N_study=N_study, subclass_name=MAPPED_TRAIT)], by.x="subclass_id", by.y="efoId", all.x=T, all.y=F)
...
gwas_efo_ids <- unique(efo_gwas$efoId)
...
n_study=col_integer(), study_N_mean=col_integer(), n_snp=col_integer(), geneNtrait=col_integer(), traitNgene=col_integer(), pvalue_mlog_median=col_double(),
...
knitr::kable(trait_top[order(-N_genes), .(MAPPED_TRAIT, N_study, N_genes)][1:18], caption="Most-polygenic traits")
```
...
" ; pVal=", sprintf("%.2g",10^(-pvalue_mlog_max)), "RCRAS = ", rcras, "; ", 
"meanRankScore = ", meanRankScore, "; ","meanRank = ", meanRank, "; ")]
