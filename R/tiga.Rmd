...
* BETA not comparable for different units, thus currently only OR used for TIGA.
* Can disease-gene associations be enriched by tissue-specificity via methods such as [NetWAS](https://hb.flatironinstitute.org/netwas)? If yes, should such a method be integrated with TIGA or perhaps supported as a post-processing step? Or/and is POSTGAP (Ensembl) already considering such sources.
...
plot_ly(gwas_platforms_cast, x=~YEAR, y=~NR, name="NR",
  ...
  layout(title = 'GWAS counts by year &amp; vendor',
         xaxis = list(title="", showgrid=F, range=c(2005, 2017), dtick=1, tickangle=45),
         yaxis = list(title="", showgrid=F),
         margin = list(t=100, l=100),
         font = list(family="monospace", size=18),
         legend = list(x=.1, y=.7), showlegend=T) %>%
  add_annotations(text=("N = count(study_accession IDs)"), showarrow=F, x=0.05, y=1, xref="paper", yref="paper")
```
...
plot_ly(gwas, x=~YEAR, y=~study_N, type="box") %>%
    layout(title="GWAS sample sizes",
         xaxis=list(title="", tickangle=45), yaxis=list(title="N_Study", type="log"),
         margin = list(t=100, l=100),
         font = list(family="monospace", size=18)) %>%
  add_annotations(text=sprintf("median:%d  mean:%d  range:[%d-%d]", median(gwas$study_N), round(mean(gwas$study_N)), min(gwas$study_N), max(gwas$study_N)), showarrow=F, x=0.5, y=1, xref="paper", yref="paper")
```
...
knitr::kable(t2g[(ok)][1:10][, .(GSYMB=geneSymbol, name=geneName, family=geneFamily, geneIdgTdl, n_study, study_N_mean, rcras, or_median, pvalue_mlog_median, geneMuScore, geneMuRank)], caption=sprintf("TIGA: %s, top hits", query_trait))
```
