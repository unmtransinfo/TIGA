...
t2g <- gt[gt$trait_uri == trait_uri,]
t2g <- t2g[!is.na(t2g$gsymb),]

t2g <- t2g[order(t2g$gsymb),]
...
t2g$specificity <- (1 / t2g$n_traits_g)
t2g$tdl <- as.character(t2g$tdl)
t2g$markersize <- 20*log(t2g$n_study+1)
t2g$text <- paste0(t2g$gsymb, "<br>", t2g$name, "<br>",	"Family: ", t2g$fam, "<br>", "TDL: ", t2g$tdl, "<br>",       "n_traits_this_gene = ", t2g$n_traits_g, " ; n_snp = ", t2g$n_snp, " ; n_study = ", t2g$n_study)
                 
t2g$tdl[is.na(t2g$tdl)] <- "NA"
#
p1 <- plot_ly(type = 'scatter', mode = 'markers') %>%
  add_markers(x = t2g$specificity[t2g$tdl=="Tclin"],
    y = t2g$or_median[t2g$tdl=="Tclin"],
    name = "Tclin",
    marker = list(symbol = "circle", color = t2g$tdl_color[t2g$tdl=="Tclin"], size = t2g$markersize[t2g$tdl=="Tclin"]),
    text = t2g$text[t2g$tdl=="Tclin"]
  ) %>%
  add_markers(x = t2g$specificity[t2g$tdl=="Tchem"],
              y = t2g$or_median[t2g$tdl=="Tchem"],
              name = "Tchem",
              marker = list(symbol = "circle", color = t2g$tdl_color[t2g$tdl=="Tchem"], size = t2g$markersize[t2g$tdl=="Tchem"]),
              text = t2g$text[t2g$tdl=="Tchem"]
  ) %>%
  add_markers(x = t2g$specificity[t2g$tdl=="Tbio"],
              y = t2g$or_median[t2g$tdl=="Tbio"],
              name = "Tbio",
              marker = list(symbol = "circle", color = t2g$tdl_color[t2g$tdl=="Tbio"], size = t2g$markersize[t2g$tdl=="Tbio"]),
              text = t2g$text[t2g$tdl=="Tbio"]
  ) %>%
  add_markers(x = t2g$specificity[t2g$tdl=="Tdark"],
              y = t2g$or_median[t2g$tdl=="Tdark"],
              name = "Tdark",
              marker = list(symbol = "circle", color = t2g$tdl_color[t2g$tdl=="Tdark"], size = t2g$markersize[t2g$tdl=="Tdark"]),
              text = t2g$text[t2g$tdl=="Tdark"]

  ) %>%
  add_markers(x = t2g$specificity[t2g$tdl=="NA"],
              y = t2g$or_median[t2g$tdl=="NA"],
              name = "NA",
              marker = list(symbol = "circle", color = t2g$tdl_color[t2g$tdl=="NA"], size = t2g$markersize[t2g$tdl=="NA"]),
              text = t2g$text[t2g$tdl=="NA"]
  ) %>%
...
for (i in 1:length(trait_uris))
{
  ...
  t2g <- gt[gt$trait_uri == trait_uri,]
  trait <- as.character(t2g$trait[t2g$trait_uri == trait_uri][1])
...
p2 <- subplot(nrows = 2, shareX = T, shareY = T, titleX = T, titleY = T, margin = 0.03,
  ...
  layout(title = paste0("GWAS Catalog TRAITS:<br>",
	...
	annotations = list(x = c(0, 0.5, 0, 0.5), y = c(0.9, 0.9, 0.4, 0.4), 
	  xanchor = "left",
	  text = paste0(trait_ids, "<br>", traits), showarrow = F,
