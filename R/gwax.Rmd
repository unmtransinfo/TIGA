...
plot_ly(type="pie", values=tbl, labels=paste0(names(tbl), " (", tbl[names(tbl)], ")"), textinfo="label+percent", textfont=list(size=12) ) %>%
  layout(showlegend=F)
```
...
quantile(pmin(assn$UPSTREAM_GENE_DISTANCE, assn$DOWNSTREAM_GENE_DISTANCE, na.rm=T), na.rm=T)
```
