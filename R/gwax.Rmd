...
plot_ly(gwas, type="histogram", x=~study_N, marker=list(color="#CC2222")) %>%
  layout(title="GWAS sample sizes",
         xaxis=list(title="Sample size", type="normal"), yaxis=list(title="N_Studies", type="log"),
         margin = list(t=100, l=100),
         font = list(family="monospace", size=18)) %>%
...
#	by.x="tcrdGeneSymbol", by.y="gsym", all.x=T, all.y=F)
...
knitr::kable(tcrd[(in_gwascat), .N, by="TDL"], caption="Gene counts by TDL")
```
