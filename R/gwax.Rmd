...
from aggregated statistics, linking a protein-coding gene and phenotype. Appropriate
thresholds will depend on use cases.

* Updated to [GWAS Catalog](https://www.ebi.ac.uk/gwas/) v1.0.2, Sept 30, 2018.
* Workflow designed for integration with TCRD & Pharos
* iCite Relative Citation Ratio (RCR) integration (in development)
* EFO subclass linking of related studies (in development)
* Prototype RShiny app: <http://unmtid-shinyapps.net/gwax/>

Issues/to-do:

* Confidence scoring for SNP-gene mappings ("auto-semi-fine mapping" possible?)
...
_(BETA not comparable for different units, thus currently only OR used for GWAX.)_
...
New publications may lack iCite RCR. Shold we impute RCR=median as reasonable prior?
...
gwas_icite[!(journal %in% gwas_icite[, .N, by=journal][order(-N)][1:7, journal]), journal := "Other"]
...
plot_ly(gwas_icite, x=~year, y=~relative_citation_ratio, color = ~journal, type="box",
  ...
  layout(title = 'iCite Relative Citation Ratio<br>GWAS Publications',
         xaxis = list(title="", showgrid=F),
         yaxis = list(title="RCR", showgrid=T),
...
___TO DO:___ Add EFO subclass based evidence aggregation scores.
