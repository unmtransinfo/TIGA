...
* BETA not comparable for different units, thus currently only OR used for GWAX. (But maybe can be aggregated if units agree.)
...
* Can disease-gene associations be enriched by tissue-specificity via methods such as [NetWAS](https://hb.flatironinstitute.org/netwas)? If yes, should such a method be integrated with GWAX or perhaps supported as a post-processing step?
...
* Use Ensembl IDs (from API) rather than gene symbols for more accurate SNP to gene mapping.
...
gt_statsfile <- "data/gt_stats_mu.tsv.gz"
...
```{r}
tbl <- table(assn$CONTEXT)
tbl <- data.table(name = names(tbl), N = as.integer(tbl))
tbl <- unique(tbl[grepl(";", name), `:=`(name = "Multiple", N = sum(N))])
plot_ly(type="pie", values=tbl$N, labels=paste0(tbl$name, " (", tbl$N, ")"), textinfo="label+percent", textfont=list(size=12) ) %>%
  layout(title="Association/SNP functional classes (aka CONTEXT)",
...
The API provides additional data including Ensembl IDs for mapped genes, from which we query the Ensembl API
...
setDT(ensbInfo)
biotypes <- ensbInfo[, .(.N), by="biotype"][order(-N)]
knitr::kable(biotypes[, Percent := round(100 * N / sum(N), 1)][N >= 100])
```
...
trait_top <- unique(merge(trait_top, gt[, .(N_genes=uniqueN(gsymb)), by="trait_uri"], by.x="TRAIT_URI", by.y="trait_uri", all.x=T, all.y=F))
knitr::kable(trait_top[order(-N_study), .(TRAIT, N_study, N_genes)][1:18], caption="Most-highly studied traits")
```
...
"OR = ", or_median, "; ", "study_N = ", study_N_mean, "; ",
...
```{r}
setorder(t2g, mu_rank)
knitr::kable(t2g[(ok)][1:10][, .(gsymb, name, fam, tdl, n_study, rcras, or_median, pvalue_mlog_median, mu_score, mu_rank)], caption=sprintf("GWAX: %s, top hits", query_trait))
```
