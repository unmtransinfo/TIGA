...
* Confidence higher for `reported` or `mapped`? Current guess: `reported`.
* EFO subclass linking of related studies.
...
setDT(efo2do)
efo2do$curie_id = sub(":", "_", efo2do$curie_id, fixed=T)
efo2do$mapped_curie = sub(":", "_", efo2do$mapped_curie, fixed=T)
setnames(efo2do, old=c("curie_id",  "mapped_curie"), new=c("efo_id", "do_id"))
gwas_efo_ids <- unique(sub("^.*/", "", efo_gwas$trait_uri))
sprintf("Total GWAS EFO_IDs: %d ; mapped to DO_IDs: %d", 
        length(gwas_efo_ids),
        length(intersect(gwas_efo_ids, efo2do$efo_id)))
```
