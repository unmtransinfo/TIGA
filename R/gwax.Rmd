...
efo <- efo[node_or_edge=="node"][, `:=`(node_or_edge=NULL, source=NULL, target=NULL)]
sprintf("EFO classes: %d ; subclass relationships: %d", uniqueN(efo$uri), nrow(efo_sub))
efo[, in_gwascat := as.logical(uri %in% trait$TRAIT_URI)]
efo_counts <- efo[, Ontology := sub("_.*$", "", id)][, .(N_in_gwas = sum(in_gwascat), N_total = .N), by=Ontology][order(-N_in_gwas)]
...
trait <- gt$trait[gt$trait_uri==query_uri][1]
sprintf("%s: %s", query_uri, trait)
```
...
plot_ly(type='scatter', mode='markers', data=t2g[(ok)], 
  ...
  layout(xaxis=list(type="log", title="Evidence (n_study)"), 
        ...
        title=paste0("GWAX: ", trait, "<br>(GWASCatalog TRAIT: ", sub('^.*/', '', query_uri), ")"),
